...
{	
    ...
    passengers[passenger].num_requests++;
	//This is where we queue the passenger into the elevator queue
    if (highest_request < passengers[passenger].num_requests) 
        highest_request = passengers[passenger].num_requests;
	...
	while(riding) {
		...
		if(elevators[elevator].floor == to_floor && elevators[elevator].state == ELEVATOR_OPEN) {
			...
			passenger[passengers].end_time=((double) time_val.tv_sec + (double) 1e-6 * time_val.tv_usec );// + (getWeight(passengers[passenger].num_requests) +1); 
...
double getWeight(double requests) {
  //  double max = 5;
    //double weight = (double)((double)( requests*requests )/ (highest_request+1)) * max;
//    weight = weight + 
